...
const WeatherCard = ({ selectedCity, onRemove }) => {
    ...
    useEffect(() => {
        ...
        fetch(`https://api.openweathermap.org/data/2.5/weather?q=${selectedCity}&appid=e28c4972540ee957319d58a96783ad41`)
            .then(response => {
                if (!response.ok) {
    ...
    if (showDetailPage) {
        return (
            <div>
                {/* Implement code to navigate to detailed weather page */}
            </div>
        );
    }
        ...
        <div className="max-w-md mx-auto mt-6 relative">
            <div className="absolute top-0 right-0 z-10">
                {showDropdown && (
                    <div className="bg-white shadow-md rounded-md mt-2 mr-2">
                        <button className="block w-full text-left px-4 py-2 text-gray-800 hover:bg-gray-200" onClick={handleDetailClick}>Detaya Git</button>
                        <button className="block w-full text-left px-4 py-2 text-gray-800 hover:bg-gray-200" onClick={handleRemoveClick}>Kaldır</button>
                    </div>
                )}
                <button className="absolute top-0 right-0 mt-2 mr-2 z-10 text-sm text-white border border-white px-3 py-1 rounded-md bg-transparent hover:bg-white hover:text-gray-800" onClick={handleDropdown}>...</button>
            </div>
            <div className="relative rounded-lg overflow-hidden shadow-lg bg-gradient-to-br from-gray-800 to-gray-600 text-white">
                ...
                <div className="absolute top-0 left-0 right-0 bottom-0 flex flex-col justify-center items-center p-4">
                    ...
                    <div className="text-base mb-2">{weatherData.weather[0].description}</div>
